...
yoCount ('Y':'o':s) = 1 + yoCount (s)-- a string starting with "Yo". What to do here?
yoCount s = yoCount (drop 1 s) -- a string that doesn't start with "yo" or "Yo". What to do here?
...
interestTable :: (Integral a1, Integral a, RealFrac a2) => a2 -> a2 -> a -> [a1]
interestTable p r 0 = [round p]
interestTable p r n = interestTable p r (n-1) ++ [round pnew]
    where pnew = p*(1+ 0.01*r)^n
          -- pold = last (interestTable p r (n-1))

-- your function here (may need more patterns!)
...
numListToNum [] = read "" :: Int
numListToNum [x] = x
numListToNum (x:xs) = x * (10^length (xs)) + numListToNum xs -- put your code here (more patterns needed?)
...
wordsToPhone w = numListToNum [charToPhoneDigit x|x <- w, x `elem` "abcdefghijklmnopqrstuvwxyzQWERTYUIOPASDFGHJKLZXCVBNM"]
--wordsToPhone (x:xs) = numListToNum ((charToPhoneDigit x):wordsToPhone xs) -- put your code here
